...
/**
 * Standard Page / Component Content Loader
 */
export function PageLoader({ text = 'Loading data...', size = 'md', fullScreen = false }) {
  const sizeClasses = {
    sm: 'w-10 h-10',
    md: 'w-16 h-16',
    lg: 'w-24 h-24',
  ...
  const content = (
    <div className="flex flex-col items-center justify-center p-8 text-center select-none">
      <div className="relative flex items-center justify-center mb-4">
        {/* Outer glowing ring */}
        <div
          className={`${sizeClasses[size]} border-4 border-emerald-100 border-t-emerald-600 rounded-full animate-spin`}
        />
        {/* Inner reverse spinning ring */}
        <div
          className={`absolute ${size === 'lg' ? 'w-16 h-16' : size === 'md' ? 'w-10 h-10' : 'w-6 h-6'} border-3 border-teal-200 border-b-teal-500 rounded-full animate-spin-reverse`}
        />
        {/* Central pulsing Agri icon */}
        <motion.div
          animate={{ scale: [0.9, 1.15, 0.9] }}
          transition={{ duration: 1.8, repeat: Infinity, ease: 'easeInOut' }}
          className={`absolute text-emerald-600 ${iconSizes[size]}`}
        >
          <MdAgriculture />
        </motion.div>
      </div>
...
/**
 * Fullscreen Action Overlay Loader (e.g. submitting form / processing order)
 */
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 backdrop-blur-sm transition-opacity">
      <motion.div
        ...
        className="bg-white px-8 py-6 rounded-2xl shadow-2xl flex flex-col items-center border border-gray-100 max-w-sm w-full mx-4"
      >
        <div className="relative flex items-center justify-center mb-4">
          <div className="w-14 h-14 border-4 border-emerald-100 border-t-emerald-600 rounded-full animate-spin" />
          <div className="absolute text-emerald-600 text-xl">
            <MdAgriculture />
          </div>
        </div>
 ...
 */
export function CardSkeleton({ count = 4, columns = 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4' }) {
  return (
    <div className={`grid ${columns} gap-6`}>
      {Array.from({ length: count }).map((_, index) => (
        <div
          key={index}
          className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-3 animate-pulse"
        >
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-gray-200 animate-shimmer" />
...
export function TableSkeleton({ rows = 5, cols = 5 }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden p-4 space-y-4">
      <div className="h-6 bg-gray-200 rounded w-1/4 animate-shimmer mb-4" />
        ...
        {Array.from({ length: rows }).map((_, rowIndex) => (
          <div key={rowIndex} className="flex gap-4 items-center">
            {Array.from({ length: cols }).map((_, colIndex) => (
              <div
                key={colIndex}
                className="h-5 bg-gray-100 rounded flex-1 animate-shimmer"
              />
            ))}
          </div>
        ))}
